...
export default async function LoginPage() {
  ...
  return (
    <main className="ambient-emerald mx-auto flex min-h-screen max-w-md flex-col justify-center gap-6 p-6">
      ...
      <Card className="animate-in" elevated>
        ...
        <div className="stagger flex flex-col gap-2">
          {db.users.map((u) => (
            <form key={u.id} action={login}>
              ...
              <button type="submit"
                className="group themed flex w-full items-center gap-3 rounded-lg px-3 py-3 text-left btn-secondary">
                <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-sm font-bold transition-transform duration-200 group-hover:scale-110"
                ...
                <span aria-hidden className="transition-transform duration-200 group-hover:translate-x-1" style={{ color: "var(--color-primary)" }}>→</span>
              </button>
            </form>
          ))}
        </div>
      ...
      <p className="animate-in text-center text-xs" style={{ color: "var(--color-text-secondary)" }}>
